...
class ChatLine(ui.EditLine):
	...
	def __PrevLastSentenceStack(self):
		...
		if gcGetEnable("ENABLE_RECOVER_MESSAGES"):
			if GetWindowConfig("system", "chat", "MESSAGES_STORAGE") and self.lastSentencePos < len(GetWindowConfig("system", "chat", "MESSAGES_STORAGE")):
				self.lastSentencePos += 1
				lastSentence = GetWindowConfig("system", "chat", "MESSAGES_STORAGE")[-self.lastSentencePos]
	...
	def __NextLastSentenceStack(self):
		...
		if gcGetEnable("ENABLE_RECOVER_MESSAGES"):
			if GetWindowConfig("system", "chat", "MESSAGES_STORAGE") and self.lastSentencePos > 1:
				self.lastSentencePos -= 1
				lastSentence = GetWindowConfig("system", "chat", "MESSAGES_STORAGE")[-self.lastSentencePos]
				self.SetText(lastSentence)
				self.SetEndPosition()
	...
	def __PushLastSentenceStack(self, text):
		...
		if gcGetEnable("ENABLE_RECOVER_MESSAGES"):
			if len(GetWindowConfig("system", "chat", "MESSAGES_STORAGE")) > LAST_SENTENCE_STACK_SIZE:
				GetWindowConfig("system", "chat", "MESSAGES_STORAGE").pop(0)
		else:
			if len(self.lastSentenceStack) > LAST_SENTENCE_STACK_SIZE:
				self.lastSentenceStack.pop(0)

		if gcGetEnable("ENABLE_RECOVER_MESSAGES"):
			GetWindowConfig("system", "chat", "MESSAGES_STORAGE").append(text)
		else:
...
class ChatWindow(ui.Window):
	...
	EDIT_LINE_HEIGHT = 25
	CHAT_WINDOW_WIDTH = 640 if (gcGetEnable("ENABLE_NEW_LOGS_CHAT")) else 620
	...
	def __init__(self):
		...
		if (gcGetEnable("ENABLE_NEW_LOGS_CHAT")):
			self.btnLogsChat = ui.Button()
			self.btnLogsChat.SetParent(self)
			self.btnLogsChat.SetUpVisual("assets/ui/logschat/log_chat_button_norm.tga")
			self.btnLogsChat.SetOverVisual("assets/ui/logschat/log_chat_button_hover.tga")
			self.btnLogsChat.SetDownVisual("assets/ui/logschat/log_chat_button_down.tga")
			self.btnLogsChat.SetToolTipText(localeInfo.CHAT_OPEN_NEW_LOGS_CHAT)
			self.btnLogsChat.Hide()
		...
		btnFilterChat.SetUpVisual("d:/ymir work/ui/game/taskbar/ignore_button_01.sub")
		btnFilterChat.SetOverVisual("d:/ymir work/ui/game/taskbar/ignore_button_02.sub")
		btnFilterChat.SetDownVisual("d:/ymir work/ui/game/taskbar/ignore_button_03.sub")
		btnFilterChat.SetToolTipText("Filtr Chat")
		...
		if gcGetEnable("ENABLE_CHAT_TIME_BOX"):
			## TimeBox
			self.timeBox = ui.ThinBoard()
			self.timeBox.SetParent(self.btnChatSizing)
			self.timeBox.SetSize(0, 40)
			self.timeBox.SetWindowHorizontalAlignCenter()
			self.timeBox.SetPosition(0, -50)

			self.timeValue = ui.MakeTextLine(self.timeBox)
			self.timeValue.SetPosition(-1, -1)
			self.timeValue.SetPackedFontColor(0xffEC8381)
			self.timeValue.SetText(time.strftime("%Y-%m-%d %H:%M:%S"))

		self.Refresh()
		self.chatInputSet.RefreshPosition()


	def SelectLang(self, argIndex, argLang):
		status = self.button_lang[argLang].GetStatus()
		if status:
			self.button_lang[argLang].SetCheck(0)
		else:
			self.button_lang[argLang].SetCheck(1)
	...
	def SaveLangs(self):
		...
		fLangsList = []
		for slot in self.button_lang:
			if not self.button_lang[slot].GetStatus():
				fLangsList.append(slot)

		cfg.Set(cfg.SAVE_GENERAL, "fLangs", str(fLangsList))

	def LoadLangs(self):
		app.SetFLang("clear")
		fLangs = cfg.Get(cfg.SAVE_GENERAL, "fLangs", "0")

		for slot in self.button_lang:
			if slot in fLangs:
				self.button_lang[slot].SetCheck(0)
				app.SetFLang(str(slot))
			else:
				self.button_lang[slot].SetCheck(1)
	...
	def OpenChat(self):
		...
		self.wndFilterChat.GetGlobalPositionByChatParent(x + width + 70, y + self.GetHeight() - 25)
		self.wndFilterChat.SetTop()
		self.wndFilterChat.UpdateRect()
		self.wndFilterChat.Hide()

		self.btnFilterChat.SetPosition(self.GetWidth() - 92, 2)
		self.btnFilterChat.Show()

		if (gcGetEnable("ENABLE_NEW_LOGS_CHAT")):
			self.btnSendWhisper.SetPosition(self.GetWidth() - 70, 2)
			self.btnChatLog.SetPosition(self.GetWidth() - 48, 2)

			if self.btnLogsChat:
				self.btnLogsChat.SetPosition(self.GetWidth() - 25, 2)
				self.btnLogsChat.Show()
		else:
	...
	if (gcGetEnable("ENABLE_NEW_LOGS_CHAT")):
		def SetMainLogsChatWindowEvent(self, event):
			if self.btnLogsChat:
				self.btnLogsChat.SetEvent(event)
	...
	def __RefreshSizingBar(self):
		...
		if gcGetEnable("ENABLE_CHAT_TIME_BOX"):
			## Time
			self.timeBox.SetSize(width - 80, self.timeBox.GetHeight())
	...
	def SetSize(self, width, height):
		...
		# self.btnFilterChat.SetPosition(width - 95, height - 23)
		self.__RefreshSizingBar()
...
LOCALE_PATH_FLAG = "assets/ui/login/languages/%s_down.png"

class ChatFilterWindow(ui.Board):
	WND_WIDTH = 70
	WND_HEIGHT = 184
	...
	def AddImageBox(self, parent, x, y, fileName):
		image = ui.MakeExpandedImageBox(parent, fileName, x, y)
		image.SetScale(0.85, 0.7)
		return self.imageBoxList.append(image)

	def GetGlobalPositionByChatParent(self, chatPosX, chatPosY):
		self.SetPosition(chatPosX - self.WND_WIDTH, chatPosY - self.Y_SPACE * (app.LANGUAGE_MAX_NUM))
		self.UpdateRect()

	def __LoadWindow(self):
		for i in xrange(app.LANGUAGE_MAX_NUM):
			self.checkBoxDataList.append(ui.ChatFilterCheckBoxLanguage(self, i, 9, self.ADD_SPACE + (self.Y_SPACE * i),
																		Event(self.OnSelectLanguage, i)))

			self.AddImageBox(self.checkBoxDataList[i], self.X_START, 2,
								LOCALE_PATH_FLAG % app.GetLanguageByID(i, True))

		for i in xrange(app.LANGUAGE_MAX_NUM):
			bEnabled = cfg.Get(cfg.SAVE_GENERAL, "language_chat_{}".format(app.GetLanguageByID(i, True)), str(1))
			if int(bEnabled):
				self.SetChatFilter(i, 1)

		self.SetSize(self.WND_WIDTH + 31, self.Y_SPACE * (app.LANGUAGE_MAX_NUM) + self.ADD_SPACE * 2 + 2)
		self.Show()

	def OnSelectLanguage(self, bLanguageID, state):
		if bLanguageID == app.GetLanguage():
			self.checkBoxDataList[app.GetLanguage()].SetState(1)
			return
	...
	def OnCheckLanguages(self, bLanguageGlobal=False):
		baseDict = {i: True for i in xrange(app.LANGUAGE_MAX_NUM)}
		...
		for i in xrange(app.LANGUAGE_MAX_NUM):
			if self.checkBoxDataList[i].IsChecked():
				cfg.Set(cfg.SAVE_GENERAL, "language_chat_{}".format(app.GetLanguageByID(i, True)), 1)
			else:
				cfg.Set(cfg.SAVE_GENERAL, "language_chat_{}".format(app.GetLanguageByID(i, True)), 0)
	...
	def OnUpdate(self):
		...
		for i in xrange(app.LANGUAGE_MAX_NUM):
			if self.checkBoxDataList[i].IsIn():
				...
				self.toolTipDialog.ClearToolTip()
				self.toolTipDialog.SetThinBoardSize(self.WND_TOOLTIP_WIDTH, 5)
				self.toolTipDialog.SetPosition(mouseX + self.X_PARSER_TOOLTIP, mouseY)
				# self.toolTipDialog.RectSize(self.WND_TOOLTIP_WIDTH, 0)
				self.toolTipDialog.AppendDescription(app.GetLanguageByID(i, False).upper() or "", None, 0xFFffffff)
				self.toolTipDialog.ResizeToolTip()
