...
class GameButtonWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self, filename):
		...
		try:
			...
			self.textButtonDict = {
				"STATUS" : self.GetChild("StatusPlusLabel"),
				"SKILL" : self.GetChild("SkillPlusLabel"),
				"QUEST" : self.GetChild("QuestButtonLabel"),
				# "HELP" : self.GetChild("HelpButtonLabel"),
				"BUILD" : self.GetChild("BuildGuildBuildingButtonName"),
				"EXIT_OBSERVER" : self.GetChild("ExitObserverButtonName"),
			}

			self.gameButtonDict["EXIT_OBSERVER"].SetEvent(ui.__mem_func__(self.__OnClickExitObserver))

			for key, button in self.gameButtonDict.items():
				button.SetOverEvent(self.OnMouseOverIn, key)
				button.SetOverOutEvent(self.OnMouseOverOut, key)

				self.textButtonDict[key].Hide()
	...
	def OnMouseOverIn(self, arg):
		self.textButtonDict[arg].Show()

	def OnMouseOverOut(self, arg):
		self.textButtonDict[arg].Hide()
