...
class MainStream(object):
	...
	def __init__(self):
		...
		net.SetUDPRecvBufferSize(64 * 4096)
		...
		image = ui.AniImageBox()
		image.AddFlag("not_pick")
		image.SetSize(512, 512)
		image.SetPosition(0, 0)

		images = ["assets/ui/loading/new/img{:04d}.png".format(i) for i in range(1, 32)]
		image.AppendImage( tuple(images) )
	...
	def SetPhaseWindow(self, newPhaseWindow):
		...
		if self.curPhaseWindow:
			if self.curPhaseWindow.__class__.__name__ == "GameWindow" and newPhaseWindow.__class__.__name__ == "LoadingWindow":
				self.__ChangePhaseWindow()
			else:
	...
	def SetSelectCharacterPhase(self):
		...
		import introLoading
		introLoading.LoadingWindow.RegisterTitleName()
	...
	def SetLoginInfo(self, id, pwd):
		self.id = id
		self.pwd = pwd
		net.SetLoginInfo(id, pwd, app.GetLanguage())
