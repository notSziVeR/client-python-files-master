...
class PartyMemberInfoBoard(ui.ScriptWindow):
	...
	if gcGetEnable("ENABLE_UPDATE_PARTY"):
		MEMBER_BUTTON_LEADER = 13
	...
	if gcGetEnable("ENABLE_UPDATE_PARTY"):
		MEMBER_BUTTON_IMAGE_FILE_NAME_DICT = {
												...
												MEMBER_BUTTON_LEADER : "party_state_leader",
		}
	else:
		MEMBER_BUTTON_IMAGE_FILE_NAME_DICT = {
	...
	def __CreateStateButton(self):
		for key, name in self.MEMBER_BUTTON_IMAGE_FILE_NAME_DICT.items():
			if not gcGetEnable("ENABLE_UPDATE_PARTY"):
		...
		if gcGetEnable("ENABLE_UPDATE_PARTY"):
			self.stateButtonDict[self.MEMBER_BUTTON_LEADER].SetEvent(ui.__mem_func__(self.OnSelectState), -1)
			self.stateButtonDict[self.MEMBER_BUTTON_LEADER].SetToolTipText(localeInfo.PARTY_SET_NORMAL)
	...
	def __AppendStateButton(self, x, y, state):
		if state == self.state:
			if gcGetEnable("ENABLE_UPDATE_PARTY"):
				if player.IsPartyLeaderByPID(self.pid):
					button = self.stateButtonDict[self.MEMBER_BUTTON_LEADER]
				else:
	...
	def SetCharacterState(self, state):
		...
		if gcGetEnable("ENABLE_UPDATE_PARTY"):
			...
			elif player.IsPartyLeaderByPID(self.pid):
				name = self.MEMBER_BUTTON_IMAGE_FILE_NAME_DICT[self.MEMBER_BUTTON_LEADER]
			else:
	...
	def OnAffectOverIn(self, index):
		...
		self.affectToolTip.SetTitle(self.AFFECT_STRING_DICT[index].format(self.affectValueDict[index]))
...
class PartyWindow(ui.Window):
	...
	def UpdatePartyMemberInfo(self, pid):
		...
		if not gcGetEnable("ENABLE_POISON_GAUGE"):
		...
		if None != vid:
			if player.IsMainCharacterIndex(vid):
				isLeader = player.IsPartyLeaderByPID(pid) if gcGetEnable("ENABLE_UPDATE_PARTY") else player.PARTY_STATE_LEADER == player.GetPartyMemberState(pid)
			...
			if gcGetEnable("ENABLE_POISON_GAUGE"):
				if chrmgr.HasAffectByVID(vid, chr.AFFECT_POISON):
					board.MakeGaugeColor("lime")
				else:
					board.MakeGaugeColor("red")
