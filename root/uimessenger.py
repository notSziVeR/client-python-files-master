...
if gcGetEnable("SAVE_WND_POSITION_MESSENGER"):
	import cfg
...
if app.ENABLE_TEAMLER_STATUS:
	class MessengerTeamGroup(MessengerGroupItem):
		...
		def __init__(self, getParentEvent):
			MessengerGroupItem.__init__(self, getParentEvent)
			self.SetName(localeInfo.MESSENGER_TEAM_TAB)
			self.AddFlag("float")

			stateImage = ui.ImageBoxNew()
			stateImage.SetParent(self)
			stateImage.SetEvent(self.OnClickStateImage, "MOUSE_LEFT_BUTTON_DOWN")
			stateImage.Show()
		...
		def OnClickStateImage(self):
			name = player.GetName()
			# if ("[GM]" in name) or ("[TGM]" in name):
			# 	return

			constInfo.SET_SHOW_TEAMLER = name
			if player.IsShowTeamler():
				net.SendChatPacket("/set_is_show_teamler 0")
				constInfo.IS_SHOW_TEAMLER_VAL = 0
			else:
				net.SendChatPacket("/set_is_show_teamler 1")
				constInfo.IS_SHOW_TEAMLER_VAL = 1

		def RefreshStateImage(self):
			if player.IsGameMaster():
				if player.IsShowTeamler():
					self.stateImage.LoadImage(self.STATE_IMAGE_FILE_NAME["ON"])
				else:
					self.stateImage.LoadImage(self.STATE_IMAGE_FILE_NAME["OFF"])
...
class MessengerWindow(ui.ScriptWindow):
	...
	def OnRemoveList(self, groupIndex, key):
		...
		if gcGetEnable("ENABLE_LEFT_POPUP"):
			if key in constInfo.ALREADY_NOTIFY_LIST and groupIndex == TEAM:
				interfaceModule.GetInstance().LoadAppLeftTip(key, "TEAM")

				constInfo.ALREADY_NOTIFY_LIST.remove(key)
	...
	def OnLogin(self, groupIndex, key, name=None):
		...
		if gcGetEnable("ENABLE_LEFT_POPUP"):
			if not name in constInfo.ALREADY_NOTIFY_LIST:
				if groupIndex == TEAM:
					interfaceModule.GetInstance().LoadAppLeftTip(name, "TEAM")
				else:
					interfaceModule.GetInstance().LoadAppLeftTip(name, "FRIEND")
