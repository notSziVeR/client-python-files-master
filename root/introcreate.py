import ui, exception, net, grp, chr, musicInfo, snd, systemSetting, app, wndMgr,\
 localeInfo, playerSettingModule, uiScriptLocale, uiToolTip, event
...
class CreateCharacterWindow(ui.Window):
	...
	def __BindActions(self):
		[self.JobList[i].SetEvent(ui.__mem_func__(self.SelectJob), i) for i in xrange(len(self.JobList))]
		[self.shapeButtonList[i].SetEvent(ui.__mem_func__(self.SelectShape), i) for i in xrange(len(self.shapeButtonList))]
		...
		for event_type in ["MOUSE_LEFT_BUTTON_DOWN", "MOUSE_OVER_IN", "MOUSE_OVER_OUT"]:
			[self.SelectBtnNameList[i].SetEvent(ui.__mem_func__(self.EventProgress), event_type, i) for i in xrange(len(self.SelectBtnNameList))]
			[self.SelectBtnFaceList[i].SetEvent(ui.__mem_func__(self.EventProgress), event_type, i) for i in xrange(len(self.SelectBtnFaceList))]
	...
	def Open(self):
		...
		try: ui.PythonScriptLoader().LoadScriptFile(self.dlgBoard, "uiscript/createcharacterwindow.py")
		except: exception.Abort("CreateCharacterWindow.LoadScriptFile")
		...
		(self.NameList, self.JobList, self.SelectBtnNameList, self.SelectBtnFaceList) = ([], [], [], [])
		[self.NameList.append(getChild(object_name)) for object_name in ["name_warrior", "name_assassin", "name_sura", "name_shaman"]]
		[self.JobList.append(getChild(object_name)) for object_name in ["WARRIOR", "ASSASSIN", "SURA", "SHAMAN"]]
		[self.SelectBtnNameList.append(getChild(object_name)) for object_name in ["name_warrior_small", "name_assassin_small", "name_sura_small", "name_shaman_small"]]
		[self.SelectBtnFaceList.append(getChild(object_name)) for object_name in ["CharacterFace_0", "CharacterFace_1", "CharacterFace_2", "CharacterFace_3"]]
		
		(self.genderButtonList, self.shapeButtonList) = ([], [])
		[self.genderButtonList.append(getChild(object_name)) for object_name in ["gender_button_01", "gender_button_02"]]
		[self.shapeButtonList.append(getChild(object_name)) for object_name in ["shape_button_01", "shape_button_02"]]
		
		(self.statValue, self.GaugeList) = ([], [])
		...
		self.JobSex = {}
		[self.JobSex.update({index : app.GetRandom(MAN, WOMAN)}) for index in xrange(4)]
	...
	def EnableWindow(self):
		...
		for i in xrange(2):
			...
			self.genderButtonList[i].Down() if self.gender == i else self.genderButtonList[i].SetUp()
			
			self.shapeButtonList[i].Enable()
			self.shapeButtonList[i].Down() if self.shape == i else self.shapeButtonList[i].SetUp()
	...
	def SelectJob(self, job_id):
		...
		if self.EVENT_DESC_MAX_VISIBLE_LINE_COUNT >= total_line:
			self.btnPrev.Hide()
			self.btnNext.Hide()
		else:
			self.btnPrev.Show()
