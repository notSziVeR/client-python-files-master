...
class TaskBar(ui.ScriptWindow):
	...
	if app.ENABLE_PREMIUM_PRIVATE_SHOP:
		BUTTON_OFFLINE_SHOP = 6
	if app.ENABLE_ANTI_MULTIPLE_FARM:
		BUTTON_ANTI_MULTIPLE_FARM = 7
	...
	def __init__(self):
		...
		self.tooltipST.Show()
		self.tooltipEXP = uiToolTip.ToolTip()
		self.tooltipEXP.Hide()
		self.isShowAnty = False
	...
	def LoadWindow(self):
		...
		if app.ENABLE_PREMIUM_PRIVATE_SHOP:
			toggleButtonDict[TaskBar.BUTTON_OFFLINE_SHOP] = self.GetChild("OfflineShopButton")
		...
		if gcGetEnable("ENABLE_ANTY_EXP"):
			blockedExpGauge = []
			blockedExpGauge.append(self.GetChild("BlockedEXPGauge_01"))
			blockedExpGauge.append(self.GetChild("BlockedEXPGauge_02"))
			blockedExpGauge.append(self.GetChild("BlockedEXPGauge_03"))
			blockedExpGauge.append(self.GetChild("BlockedEXPGauge_04"))

			for i in xrange(len(blockedExpGauge)):
				blockedExpGauge[i].Hide()
		...
		mouseImage = ui.Button("TOP_MOST")
		mouseImage.AddFlag("float")
		mouseImage.SetUpVisual("d:/ymir work/ui/game/taskbar/mouse_button_camera_01.sub")
		mouseImage.SetOverVisual("d:/ymir work/ui/game/taskbar/mouse_button_camera_02.sub")
		mouseImage.SetDownVisual("d:/ymir work/ui/game/taskbar/mouse_button_camera_03.sub")
		...
		if gcGetEnable("ENABLE_ANTY_EXP"):
			self.expGaugeBoard.OnMouseLeftButtonDown = ui.__mem_func__(self.BlockExp)
			self.blockedExpGauge = blockedExpGauge
			self.LoadNewImages()
	...
	def SetHP(self, curPoint, recoveryPoint, maxPoint):
		...
		if maxPoint > 0:
			...
			self.tooltipHP.SetText("{} : {} / {}".format(localeInfo.TASKBAR_HP, localeInfo.GetFormattedNumberString(curPoint), localeInfo.GetFormattedNumberString(maxPoint)))
	...
	def SetSP(self, curPoint, recoveryPoint, maxPoint):
		...
		if maxPoint > 0:
			...
			self.tooltipSP.SetText("{} : {} / {}".format(localeInfo.TASKBAR_SP, localeInfo.GetFormattedNumberString(curPoint), localeInfo.GetFormattedNumberString(maxPoint)))
	...
	def SetST(self, curPoint, maxPoint):
		...
		if maxPoint > 0:
			...
			self.tooltipST.SetText("{} : {} / {}".format(localeInfo.TASKBAR_ST, localeInfo.GetFormattedNumberString(curPoint), localeInfo.GetFormattedNumberString(maxPoint)))
	...
	def SetExperience(self, curPoint, maxPoint):
		...
		if gcGetEnable("ENABLE_ANTY_EXP"):
			if GetWindowConfig("system", "anty_exp", "ANTY_EXP_STATUS_{}".format(player.GetName())) == 1:
				self.tooltipEXP.ClearToolTip()
				self.tooltipEXP.AppendTextLine("%s: %.2f%%" % (localeInfo.TASKBAR_EXP, float(curPoint) / max(1, float(maxPoint)) * 100), 0xFFffffff)
				self.tooltipEXP.AppendTextLine("{} / {}".format(localeInfo.GetFormattedNumberString(unsigned32(player.GetEXP())), localeInfo.GetFormattedNumberString(unsigned32(player.GetStatus(player.NEXT_EXP)) - unsigned32(player.GetStatus(player.EXP)))), 0xFFffffff)
				self.tooltipEXP.AppendShortcut([app.DIK_LMBUTTON], CFF.format(localeInfo.TASKBAR_ANTY_EXP_UNLOCK, 'red'), bCenter = True)
			else:
				self.tooltipEXP.ClearToolTip()
				self.tooltipEXP.AppendTextLine("%s: %.2f%%" % (localeInfo.TASKBAR_EXP, float(curPoint) / max(1, float(maxPoint)) * 100), 0xFFffffff)
				self.tooltipEXP.AppendTextLine("{} / {}".format(localeInfo.GetFormattedNumberString(unsigned32(player.GetEXP())), localeInfo.GetFormattedNumberString(unsigned32(player.GetStatus(player.NEXT_EXP)) - unsigned32(player.GetStatus(player.EXP)))), 0xFFffffff)
				self.tooltipEXP.AppendShortcut([app.DIK_LMBUTTON], CFF.format(localeInfo.TASKBAR_ANTY_EXP_BLOCK, 'green'), bCenter = True)
		else:
			self.tooltipEXP.ClearToolTip()
			self.tooltipEXP.AppendTextLine("%s : %.2f%%" % (localeInfo.TASKBAR_EXP, float(curPoint) / max(1, float(maxPoint)) * 100))
		...
		for slot in self.quickslot:
			...
			for i in xrange(4):
				...
				if player.SLOT_TYPE_INVENTORY == Type:
					...
					if item.ITEM_TYPE_TOGGLE == item.GetItemType():
						if 0 != int(metinSocket[3]):
							slot.ActivateSlot(slotNumber)
	...
	def OnUpdate(self):
		...
		if True == self.hpGaugeBoard.IsInPosition():
			self.tooltipHP.Show()
		else:
			self.tooltipHP.Hide()

		if True == self.mpGaugeBoard.IsInPosition():
			self.tooltipSP.Show()
		else:
			self.tooltipSP.Hide()

		if True == self.stGaugeBoard.IsInPosition():
			self.tooltipST.Show()
		else:
			self.tooltipST.Hide()

		if True == self.expGaugeBoard.IsIn():
			self.OnMouseOverInExp()
		else:
			self.OnMouseOverOutExp()

	def OnMouseOverInExp(self):
		if self.tooltipEXP:
			self.SetExperience(unsigned32(player.GetStatus(player.EXP)),unsigned32(player.GetStatus(player.NEXT_EXP)))
			self.tooltipEXP.Show()
			self.isShowAnty = True

	def OnMouseOverOutExp(self):
		if self.isShowAnty:
			self.tooltipEXP.Hide()
			self.isShowAnty = False
	...
	if gcGetEnable("ENABLE_ANTY_EXP"):
		def LoadNewImages(self):
			for i in xrange(len(self.expGauge)):
				if GetWindowConfig("system", "anty_exp", "ANTY_EXP_STATUS_{}".format(player.GetName())):
					self.blockedExpGauge[i].Show()
				else:
					self.blockedExpGauge[i].Hide()
				self.expGauge[i].SetSize(0, 0)
				self.SetExperience(unsigned32(player.GetStatus(player.EXP)),unsigned32(player.GetStatus(player.NEXT_EXP)))
